...
class PolygonNewsCollector:
    def __init__(self, api_key, db_path='AAPL_news_v2.db'):
    ...
    @staticmethod
    def _article_mentions_apple(article, ticker):
        """
        Retourne True si l'article parle vraiment d'Apple :
        - le ticker AAPL est dans la liste des tickers renvoyée par Polygon
        - ET le texte (titre + description) contient 'Apple' ou 'AAPL' comme mot
        """
        ...
        # Utiliser regex pour éviter 'pineapple', etc.
        if re.search(r"\bapple\b", text, flags=re.IGNORECASE):
            return True
        if re.search(r"\baapl\b", text, flags=re.IGNORECASE):
            return True
    ...
    def save_articles(self, articles, ticker):
        ...
        en filtrant pour garder ceux qui parlent vraiment d'Apple.
        """
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        for article in articles:
            try:
                ...
                if not self._article_mentions_apple(article, ticker):
                    skipped += 1
                    continue
                ...
                # Extraire les données de sentiment (Polygon si dispo)
        ...
        print(
            f"   💾 {new_articles} nouveaux articles sauvegardés, "
            f"{duplicates} doublons ignorés, {skipped} articles filtrés (pas vraiment sur Apple)"
        )
        return new_articles
    ...
    def get_stats(self):
        ...
        # Articles avec sentiment Polygon
        cursor.execute("""
            SELECT COUNT(*)
            FROM news_articles
        ...
        if total > 0:
            print(f"Articles avec sentiment Polygon: {with_sentiment} ({with_sentiment/total*100:.1f}%)")
        else:
            print("Articles avec sentiment Polygon: 0")
...
if __name__ == "__main__":
    ...
    # Créer le collecteur avec une DB dédiée aux news vraiment sur Apple
    collector = PolygonNewsCollector(api_key=API_KEY, db_path="AAPL_news_v2.db")

    # Période fixe : 1er janvier 2020 -> 10 décembre 2025
    START_DATE = "2020-01-01"
    END_DATE = "2025-12-10"

    print("\n🚀 Début de la collecte pour AAPL (2020–2025, articles vraiment sur Apple)")
    count = collector.collect_ticker_range("AAPL", START_DATE, END_DATE, total_limit=None)

    print(f"\n✅ AAPL: {count} articles 'Apple-focused' collectés entre {START_DATE} et {END_DATE}")

    # Statistiques
    collector.get_stats()

    print("\n✅ Collecte terminée! Base de données prête pour l'entraînement.")
    print(f"📁 Fichier: AAPL_news_v2.db")
