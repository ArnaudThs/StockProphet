...
def _get_cache_path(ticker: str) -> Path:
    ...
    return CACHE_DIR / f"{ticker}_sentiment.parquet"
...
def _fetch_sentiment_from_api(api_key: str, ticker: str, start_date: str, end_date: str, sleep: float = 1.0, max_retries: int = 5) -> pd.DataFrame:
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "429" in str(e) or "too many" in str(e).lower():
                wait_time = sleep * (2 ** attempt)  # Exponential backoff: 1, 2, 4, 8, 16 seconds
...
def fetch_ticker_sentiments(api_key: str, ticker: str, start_date: str, end_date: str, sleep: float = 1.0, max_retries: int = 5, force_refresh: bool = False) -> pd.DataFrame:
    """
...
def fetch_daily_ticker_sentiment(api_key: str, ticker: str, start_date: str, end_date: str, force_refresh: bool = False) -> pd.DataFrame:
    """
